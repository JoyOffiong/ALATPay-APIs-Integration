...
localStorage.setItem("transactionID", JSON.stringify(VA_Response.transactionId));
...
const confirmPayment=()=>{
    ...
    <>
     ...
     <div className='shadow-lg p-4 space-y-3 w-full bg-white'>
      ...
      ( <div className='mx-auto text-center leading-4'>
        <p className='pt-6'>Kindly make your Transfer of <strong>N100</strong> to this Account </p>
         {/* businessName */}
         {!VA_Response.virtualBankAccountNumber  ? <div className='pt-6'><CircularProgress size= "20px" color="inherit"/></div> :
         ( 
         ...
         <p className='text-lg pt-6 font-bold'>Blaqkly</p>
          ...
          <p className='text-2xl font-bold'>
            ...
            {VA_Response.virtualBankAccountNumber }</p>
          {/* Bank */}
          <p className='text-base font-light'>Wema Bank</p>

              <div className='text-center flex justify-center'>
                <button onClick={()=>ConfirmationPage()} className='bg-secondary flex gap-4 px-6 py-3 rounded-md bg-blue-600  text-white border-blue-400 text-lightBrown'>I have Transferred</button></div>
              
         
          <p className=' pt-6 space-y-4 italic text-xs font-bold'>Kindly transfer the exact amount to the account number provided. <br className='pt-2' />
          This account is valid for a single transaction and will remain active for the next 30 minutes</p>
          </div>
          )
         }
  ...
  <div className='text-center flex justify-center'>
    ...
    <LoadingButton text='I have Transferred'  onClick={confirmPayment} loading={loading}/>
               </div>
